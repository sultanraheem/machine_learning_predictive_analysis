...
sqlite3
How to Use
Place the vgsales.csv, GDP.csv, and Population.csv files in the appropriate directory.
Run the script to load, clean, and store the data.
The cleaned data will be stored in an SQLite database named games_data.sqlite with the following tables:
...
Usage
Prepare the Environment: Ensure that the required libraries are installed, and the SQLite database is correctly placed in the specified path.

Run the Script: Execute the script cell by cell if using a Jupyter notebook, or run it as a standalone Python script.

Analyze the Results: Review the output, particularly the R² and MSE values, to understand the limitations of the models.
